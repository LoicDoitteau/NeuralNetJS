...
Network.prototype.init = function(randomize) {
    if(randomize) {
        ...
        this.weigths = Array.from({length : this.layers.length - 1}, (_, i) => new Matrix(this.layers[i + 1].size, this.layers[i].size));
...
Network.prototype.backPropagate = function(learningRate, output) {
    ...
    this.weigths.forEach((x, i) => x.add(Matrix.dotProduct(this.gradients[i], Matrix.transpose(this.layers[i].neurons)).multiply(learningRate)));
    this.biases.forEach((x, i) =>  x.add(Matrix.multiply(this.gradients[i], learningRate)));
};
